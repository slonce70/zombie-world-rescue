...
const source = readFileSync(path, 'utf8')
  ...
  .replace("import { sanitizeFighterId, specialistBias } from './specialists.js';", "const sanitizeFighterId = (id, fallback = null) => ['guard','medic','scout','bastion','impulse'].includes(id) ? id : fallback; const specialistBias = (id) => id === 'guard' || id === 'bastion' ? { tags: ['tank'], ids: [], multiplier: 2 } : id === 'scout' ? { tags: ['speed'], ids: [], multiplier: 2 } : id === 'medic' ? { tags: [], ids: ['spdheal','spdfull','dmgvamp','spdvamp','vamp','vamp2'], multiplier: 2 } : null;");
