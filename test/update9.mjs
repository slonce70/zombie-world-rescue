...
console.log('▸ Зоряний шлях до 30 і косметика');
const pass = await page.evaluate(() => {
  const g = window.__game;
  g.test.addXp(50000); // вистачає на 30-й
  return {
...
check(pass.level === 30, `стеля пасса тепер 30 (рівень ${pass.level})`);
...
await page.evaluate(() => {
  const g = window.__game;
  g.endLevel();
  g.save.liberated = { UKR: true, POL: true, DEU: true, FRA: true, ESP: true, ITA: true, TUR: true, SWE: true, EGY: true, JPN: true };
  g.saveGame();
  g.test.startArena();
});
...
check(arenaRun.over && arenaRun.completed, 'усі 10 босів переможено — забіг завершено');
check(arenaRun.styles.length === 9, `усі стилі босів зустрілись (${arenaRun.styles.join(',')})`);
