...
await page.evaluate(() => {
  const g = window.__game;
  g.endLevel();
  g.save.liberated = { UKR: true, POL: true, DEU: true, FRA: true, TUR: true, EGY: true };
  g.saveGame();
  g.test.startArena();
});
...
check(arenaRun.over && arenaRun.completed, 'усі 6 босів переможено — забіг завершено');
check(arenaRun.styles.length === 6, `усі стилі босів зустрілись (${arenaRun.styles.join(',')})`);
