...
// v302 «Чистий фундамент» — розслідування росту (замір per-frame, декомпозиція сцени):
//   • статична геометрія UKR від origin ~122 (стабільно, 0 джитеру);
//   • +мінне поле/тотеми/магніт ~ +9;
//   • +2 метеор-вогні ~ +0 (МЕТЕОРИ НЕ винуватець — попередня версія коментаря помилялась);
//   • +орда: живі зомбі (36 орди + амбієнтні) — КОЖЕН окремий SkinnedMesh; поки вони
//     роумлять у 'chase', кількість у фрустумі гуляє від кадру до кадру → 143–174 за кадр.
// Отже винуватець джитеру/росту — ЗОМБІ-ОРДА (чесний видимий геймплей, який гейт і має
// стресити), а НЕ якийсь дармовий/поза-кадром меш. Названі кооп-фічі v295–v300 у цій сцені
// ВІДСУТНІ: аура — лише на елітах (тут їх 0), супер-пікап не заспавнено, піщана буря — лише
// EGY, дошка табору — у сцені бази. Тому геймплей не чіпаємо, бюджет лишаємо 165 (макс-джитер
// одного кадру + запас). Заміри: node test/mobile-perf.mjs, 2026-07-07.
