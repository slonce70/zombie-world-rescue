...
Версія релізу синхронізується у `version.json`, `APP_VERSION` та service-worker cache. Кооперативний wire format використовує `PROTO_VERSION = 23`.

Експедиція v607 має трьох спеціалістів — Захисника, Медика й Розвідника. Вони використовують наявні скіни, зброю, кнопку гаджета та кооперативні ролі; постійна майстерність зберігається окремо для кожного спеціаліста.
