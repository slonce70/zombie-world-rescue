...
export const BIOMES = {
  ...
  moon: {
    ...
    fogColor: 0x9da2ac, fogNear: 155, fogFar: 500,
    hemiSky: 0xe5e9f2, hemiGround: 0x7d838d, hemiIntensity: 1.35,
    sunColor: 0xffffff, sunIntensity: 2.7, sunPos: [110, 140, 50],
    sunDisc: 0xf4f7ff, sunDiscPos: [390, 430, 210],
    grass1: 0xd8dbe0, grass2: 0xb9bec6, grass3: 0xeff0f2,
    rock: 0xaeb3bb, peak: 0xf4f5f6, water: 0x7f858e, riverbed: 0x9ba0a8,
    dirt: 0xc4c7cc, plaza: 0xe0e2e5, arenaGround: 0xb3b7bd,
    roadMain: 0xd9dce1, roadEdge: 0x8f949c,
...
export const COUNTRIES = {
  ...
  ESP: {
    ...
    biome: 'spainSun',
    map: spainMap,
    // ⚖️ v750: середина кампанії перестала бути пласкою. Крок кривої від ESP до CHN —
    // рівні +8–10 % hp, +0.06 dmg, +0.08 counts (було +1.3 % hp, яких не відчути).
    difficulty: { hp: 1.95, dmg: 1.40, counts: 1.50 },
    coinReward: 600,
    extraZombie: 'toro',
    shieldGuards: 3,
    boss: { name: t('👑 МАТАДОР-ЗОМБІ'), hp: 5000, frost: false, style: 'matador' },
  ...
  PRT: {
    ...
    biome: 'spainSun',
    map: portugalMap,
    difficulty: { hp: 2.15, dmg: 1.46, counts: 1.58 },
    coinReward: 650,
    extraZombie: 'toro',
    shieldGuards: 3,
    boss: { name: t('👑 КАПІТАН АТЛАНТИК'), hp: 5600, frost: false, style: 'matador' },
  ...
  ITA: {
    ...
    biome: 'italyMed',
    map: italyMap,
    difficulty: { hp: 2.35, dmg: 1.52, counts: 1.66 },
    coinReward: 600,
    extraZombie: 'gladiator',
    shieldGuards: 3,
    boss: { name: t('👑 ЦЕЗАР-ЗОМБІ'), hp: 6300, frost: false, style: 'gladiator' },
  ...
  TUR: {
    ...
    biome: 'bosphorus',
    map: turkeyMap,
    difficulty: { hp: 2.55, dmg: 1.58, counts: 1.74 },
    weaponReward: 'magnum',
    weaponRewardToast: () => t('Ти отримав МАГНУМ! {k} — один постріл, один зомбі 🤠', { k: keyHint('кнопка 🔁', 'Клавіша 5') }),
    extraZombie: 'gunner',
    shieldGuards: 4,
    boss: { name: t('👑 ПАША КЕБАБ'), hp: 7100, frost: false, style: 'sultan' },
  ...
  SWE: {
    ...
    biome: 'winterDusk',
    map: swedenMap,
    difficulty: { hp: 2.75, dmg: 1.64, counts: 1.82 },
    coinReward: 700,
    extraZombie: 'snowman',
    shieldGuards: 4,
    boss: { name: t('👑 СНІГОВИЙ ЯРЛ'), hp: 7900, frost: true, style: 'frost' },
  ...
  JPN: {
    ...
    biome: 'sakura',
    map: japanMap,
    difficulty: { hp: 3.25, dmg: 1.76, counts: 1.98 },
    coinReward: 800,
    extraZombie: 'samurai',
    shieldGuards: 4,
    boss: { name: t('👑 СУМО-ЗОМБІ'), hp: 9500, frost: false, style: 'sumo' },
  ...
  CHN: {
    ...
    biome: 'greatwall',
    map: chinaMap,
    // ⚖️ v750: вершина кампанійної кривої — ×3.5 hp до України (було ×2.55).
    // На ★1 лишається прохідним: снайперка з хедшотом досі знімає вокера з пострілу.
    difficulty: { hp: 3.50, dmg: 1.82, counts: 2.06 },
    coinReward: 900,
    extraZombie: 'terracotta',
    shieldGuards: 4,
    boss: { name: t('👑 ТЕРАКОТОВИЙ ІМПЕРАТОР'), hp: 10300, frost: false, style: 'emperor' },
  ...
  LOST: {
    ...
    biome: 'prehistoric',
    map: lostIslandMap,
    // ⚖️ v750: острів відкривається ЛИШЕ після всіх 12 країн, тож він мусить лишатись
    // вищим за Китай — інакше після піднятої кривої фінал-бонус став би легшим за фінал.
    difficulty: { hp: 3.65, dmg: 1.88, counts: 2.14 },
    weaponReward: 'laser',
    weaponRewardToast: () => t('Ти отримав ЛАЗЕР! {k} — промінь крізь юрбу 🔴', { k: keyHint('кнопка 🔁', 'Клавіша 8') }),
    extraZombie: 'toro',
    shieldGuards: 4,
    boss: { name: t('👑 ЗОМБІ-ТИРАНОЗАВР'), hp: 10900, frost: false, style: 'rex' },
  ...
  LAB: {
    ...
    biome: 'slimeLab',
    map: labMap,
    // ⚖️ v750: Глава 3 йде після Острова Динозаврів — тримаємо її на крок вище (див. LOST).
    difficulty: { hp: 3.80, dmg: 1.94, counts: 2.22 },
    coinReward: 1000,
    extraZombie: 'spitter',
    shieldGuards: 4,
    boss: { name: t('🧪 МЕГА-СЛИЗНЯК'), hp: 11600, frost: false, style: 'slime' },
  ...
  MOON: {
    ...
    biome: 'moon',
    map: moonMap,
    // ⚖️ v750: Місяць — ПАРАЛЕЛЬНА гілка (доступний із глобуса без проходження кампанії),
    // тож він свідомо лишається на старій кривій. Піднімати його разом із ESP..CHN не можна:
    // дитина може висадитись на Місяць одразу після України.
