...
export const SHOP_ITEMS = [
  ...
  { id: 'mapeditorplus', icon: '🏗️', name: t('Створювач карт+'), desc: t('+1 карта, сніг або літо, різні зомбі, +20 обʼєктів, ящики з парашутом, церква, відбудова міста й бос 5500 HP'), price: 15000, max: 1, cat: t('Режими'), needsUpgrade: 'mapeditor' },
  ...
  { id: 'chainlightning', icon: GADGETS.chainlightning.icon, name: GADGETS.chainlightning.name, desc: () => GADGETS.chainlightning.desc + t(' · перезарядка {n}с', { n: GADGETS.chainlightning.cd }), price: GADGETS.chainlightning.price, max: 1, cat: t('Гаджети й друзі'), gadget: true },
  { id: 'chainlightning-hyper', icon: '⚡', name: t('Гіперзаряд: Ланцюгова блискавка'), desc: t('6 зомбі, 40 HP, 1с оглушення і 0.9с збиття атаки'), price: 5000, max: 1, cat: t('Гіперзаряди'), hyper: 'chainlightning', needsGadget: 'chainlightning' },
  { id: 'frostgrenade-hyper', icon: '⚡', name: t('Гіперзаряд: Крижана граната'), desc: t('55 шкоди, зона 15×15м і 5с заморозки'), price: 5000, max: 1, cat: t('Гіперзаряди'), hyper: 'frostgrenade', needsGadget: 'frostgrenade' },
  ...
  { id: 'dnaswitch', icon: GADGETS.dnaswitch.icon, name: GADGETS.dnaswitch.name, desc: () => GADGETS.dnaswitch.desc + t(' · перезарядка {n}с', { n: GADGETS.dnaswitch.cd }), price: GADGETS.dnaswitch.price, max: 1, cat: t('Гаджети й друзі'), gadget: true },
  { id: 'dnaswitch-hyper', icon: '⚡', name: t('Гіперзаряд: ДНК-перемикач'), desc: t('3 зомбі, 9с плутанини і +5 шкоди від плутаних зомбі'), price: 5000, max: 1, cat: t('Гіперзаряди'), hyper: 'dnaswitch', needsGadget: 'dnaswitch' },
...
// ⚡ Каталог гіперзарядів: id гаджетів, для яких у магазині взагалі існує гіперзаряд.
// Єдине джерело правди для санітизації: у коопі гість ОГОЛОШУЄ свої гіперзаряди
// (`net/coop.js` → `sanitizeRosterEntry`), і оголосити те, чого в грі немає, не має.
export const HYPER_IDS = Object.freeze([...new Set(SHOP_ITEMS.filter((i) => i.hyper).map((i) => i.hyper))]);
...
export function goalInfo(game) {
  ...
  const price = typeof item.price === 'function' ? item.price(game.save) : item.price;
  const need = item.radiationPrice || item.crystalPrice || price;
...
export class Shop {
  ...
  priceOf(item) {
    // 🌟 динамічна ціна (Пожертва рятівника росте геометрично): price може бути функцією save→число
    return typeof item.price === 'function' ? item.price(this.game.save) : item.price;
  }

  // базова (не-динамічна) ціна для порівняння «чи є націнка 📈»: у динамічних товарів беремо поточну
  basePriceOf(item) {
    return typeof item.price === 'function' ? this.priceOf(item) : item.price;
  }
