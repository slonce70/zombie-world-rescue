...
export class AudioMan {
  ...
  // 🔫 безперервний промінь лазера: короткий «дзум»-тик (гравець дросселить виклик)
  beamTick() {
    const t = this.t;
    this._osc('sawtooth', 1200, t, 0.09, 0.06, 1700);
    this._osc('square', 600, t, 0.07, 0.04, 900);
  }

  // 🔥 безперервний потік вогнемета: шиплячий шум (гравець дросселить виклик)
  flameTick() {
    this._noise(this.t, 0.12, 0.16, 'bandpass', 900, 0.7, 1300);
  }

  // континуальні зброї не тримають живий вузол — глушити нічого, але метод існує для player.js
  stopBeam() { /* короткі тики згасають самі */ }
