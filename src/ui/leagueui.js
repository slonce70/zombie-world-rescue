...
export class LeagueUI {
  ...
  async render() {
    ...
    for (const [mid, label] of [['storm', '⛈️ Шторм'], ['arena', '👑 Арена']]) {
    ...
    html += '<div id="league-list" class="league-list"><div class="league-loading">📡 Завантажуємо рекорди світу…</div></div>';
    root.innerHTML = html;
    ...
    if (!list) return;
    if (!data) {
      list.innerHTML = '<div class="league-loading">📡 Ліга недоступна — перевір інтернет і спробуй ще раз</div>';
      return;
    }
    ...
    if (!rows) rows = '<div class="league-loading">Поки порожньо — стань ПЕРШИМ у світі! 🚀</div>';
    if (data.me && !data.top.some((e) => e.me)) {
      rows += `<div class="league-row me gap"><span class="lr-rank">${data.me.rank}.</span>
        <span class="lr-nick">Ти</span><span class="lr-score">${fmt(data.me.score)}</span></div>`;
    }
    list.innerHTML = rows;
  }
