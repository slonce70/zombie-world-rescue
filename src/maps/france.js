...
import { ridge, terraces } from '../utils.js';

export default {
  bound: 200,
  spawn: { x: 0, z: 170 },
  // 🏔️ Прованс: терасований пагорб виноградника сходинками, м'які кряжі на
  // північному заході і заході — як справжні плато Провансу
  terrain: (x, z) =>
    terraces(x, z, 70, 42, 58, 10, 2.4)
    + ridge(x, z, -188, -120, -90, -188, 10, 40)
    + ridge(x, z, -188, 60, -188, 188, 7, 26),
  sites: {
  ...
  // 📖 «Куля над Парижем»: точки історії (поруч із моделями, не всередині)
  storySites: {
    bakery: { x: -92, z: -58, r: 8 },        // пекарня з кухарями = сайт порятунку
    balloonField: { x: -34, z: -66, r: 10 }, // галявина кулі (модель кулі на -40,-60 — точка поруч)
    wineCellar: { x: -110, z: 48, r: 10 },   // перед ворітьми складу (футпринт ангара 16x9 при z≈60 — точка південніше)
    cafe: { x: 18, z: 26, r: 7 },            // тераса кафе (модель на 12,20 — НПС поруч, не в стіні)
    arena: { x: 18, z: -160, r: 28 },
  },
  roads: [
