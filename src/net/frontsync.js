...
export function canonicalFrontRewards(previousValue, nextValue) {
  ...
  const transition = applyFrontEvent(previous, { type: 'CLAIM_OPERATION' });
  const expected = sanitizeFrontSnapshot(transition && transition.front);
  if (!expected || JSON.stringify(expected) !== JSON.stringify(next)) return [];
  return expandFrontRewards(sanitizeFrontRewards(transition.effects));
}
