...
// 📣 стікери — ЗАКРИТИЙ набір із 12 штук, без вільного тексту. Це головна причина,
// чому в грі стікери, а не чат: дитина не пише нічого, лише обирає — модерація не
// потрібна за побудовою. Порядок ЗАМОРОЖЕНИЙ: по мережі їде індекс, тож нові
// стікери дописуються тільки В КІНЕЦЬ, а перші п'ять лишаються на своїх місцях.
...
// шляхом, що нік і скін, а хост будує гіпер-турель і гіпер-метеорит лише тому, хто
// їх оголосив.
//
// ⚠️ МЕЖА ДОВІРИ, і вона тут навмисна. Це САМООГОЛОШЕННЯ: каталог магазину звіряє
// ФОРМУ (чи існує такий гіперзаряд у грі), а не ВОЛОДІННЯ — доказу покупки в хоста
// немає й бути не може, бо сейв живе у гостя. Модифікований клієнт може дописати
// сюди 'turret' чи 'meteor' і отримати посилену версію. Обрано так свідомо:
// альтернатива — не вірити нікому, і тоді чесний гравець, який купив гіперзаряд за
// 5000 монет, у ролі гостя його просто не бачить (платна прокачка мовчки не працює).
// Ціна брехні — трохи сильніша турель у дитячій кімнаті; ціна недовіри — зламана
// покупка. Закрити межу можна лише довіреним джерелом покупок (server-authoritative
// профіль), який свідомо відхилено в TODO.md. Поведінку закріплено тестами:
// `test/coop-gadget-limits.mjs` (крок 6c′) і `test/gadgetguard-unit.mjs`.
